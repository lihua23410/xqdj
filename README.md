...
| `地慧星` | 165 | 9999 | 天蓝色球，常驻故障切片。身前 60° 扇环弹伤 6 并叠【剑痕】，20% 留下可穿过的静止故障残影，敌人经过残影再叠一层剑痕。受击时若闪避就绪则无效该次攻击，瞬移到离敌人最远的八角笼角并朝对方打一发弹（r=12、速 600、伤 9），12s CD，开局可用，原地留残影；CD 内则确认伤害并清掉撞墙加速。每次撞场边立刻把巡航和速度 +50，真正掉血时一起拍回 165。20s 起锁定并立刻索敌，球体带短刀光蓄力 2s（开时播居合音）；判定是宽 5 倍球直径的无限长矩形，伤 `(6+剑痕×2)×(1+残影)`，命中清剑痕和残影、CD 20s，未命中 CD 10s。刀光单位活 3s |
| `教父` | 170 | 0 | 不转向。球自己不出伤。第 1/2/3 秒从场边各冒一只活随从，之后缺编用一只 2.5s 补人钟。最多三只、种类不重复。暗杀者视野 140、Pass 冲刺 640 伤 7；狙击者瞄 7s 射穿砌墙的弹伤 20；贩卖者每 5s 丢药（己方捡回血/加次数） |
...
| `盾斧` | 165 | 9999 | 灰球 r=18。平时盾/红盾，前方 ≈24.7 / 120° 摸到人改剑/红剑打蓄力二连斩（5+5，红剑每段 +2；打中战斗机能量 +1），再以 350 突刺（碰到人 3 点后第二套二连斩，1.5s 未碰到则灌瓶）。连招后 Stand 1s 灌瓶（1 白 / 已白则黄 / 2 黄），0.5s 后黄瓶变红盾→红剑→红斧。盾正面 300° −50%，红盾 −75%。红斧巡航 90，索敌 104 / 120°，每段结束后转向敌人；超解扇形与波 104 / 60°，摸到人 Stand 打大解/追解/超解后红态全掉 |

非可选单位（不进选人列表，`Fighter: false`）：`子弹`、`分身`、`分身弧`、`无下限`、`无下限术士弧`、`无下限弧`、`紫弹`、`面灵气弹`、`面灵气面具1`、`面灵气面具2`、`面灵气面具3`、`盾`、`盾碎片`、`弱化碎片`、`地慧星残影`、`地慧星斩击`、`地慧星弧`、`地慧星弹`、`原型机_近战弧`、`小骑士弧`、`收割者镰刀`、`收割者收回镰刀`、`钓鱼佬鱼塘`、`钓鱼佬场鱼塘`、`钓鱼佬鱼`、`钓鱼佬中鱼`、`钓鱼佬大鱼`、`囚徒钩爪`、`囚徒囚笼`、`囚徒绞刑架`、`囚徒电椅`、`狼人月亮`、`狼人弧`、`狼人撕咬弧`、`人偶使偶`、`人偶使炸弹`、`人偶使弹`、`人偶使激光`、`教父暗杀者`、`教父狙击者`、`教父贩卖者`、`教父狙击弹`、`教父药物`。
...
unit.Spec{
    ...
    Radius:    18,
    MaxHP:     100,
    Speed:     160,              // 开局巡航。可用 SetCruise 改。快了每 0.2s 减 10，慢了但仍在动每 0.2s 加 5；0 且未持 Stand 则立刻回到巡航
    ...
    BreakWalls: false,           // true = 击中砌墙则该截提前消失并穿过；场边仍撞
...
| `RoleMinion` | 活随从。实心、吃伤害，不计入胜负。要 `Mortal: true` |
...
- `Sense` 每帧都有（hit-stop / `Stun` 期间不发）：`Time`、`Self`（自己的快照）、`Nearby`（视野内；自己的随从始终在内）。索敌扫 `Role == unit.RoleFighter` 且 `Slot != Self.Slot`，或 `unit.EnemyFighter`。出伤打活随从用 `unit.Hittable` / `unit.EnemyTarget`。
...
| `SetCruise` | 改自己的巡航。不改当前速度。快于巡航拉回；慢于巡航但仍在动则沿当前方向以回拉一半往上推；速率为 0 且未持 `Stand` 则立刻回到巡航 |
...
这些是协助者改角色或物理时不要随便推翻的约定。巡航带子的来由见 `docs/adr/0001-cruise-is-a-band.md`。活随从见 `docs/adr/0002-living-minions-take-damage.md`。
...
- 战斗机快于巡航时每 0.2s 减 10；慢于巡航但仍在动时每 0.2s 加 5；速率为 0 且巡航大于 0 时沿巡航朝向立刻回到巡航。持有 `Stand` 时速率为 0 不推。巡航开局等于 `Spec.Speed`，视野开局等于 `Spec.Vision`，都可以随时改。拉/推发生在消化指令之前，所以当帧 `SetVelocity` 能顶住。
- 血量默认 100；`HP <= 0` 由引擎移除。`RoleFighter` 吃伤害且必须确认 `IncomingDamage`。`Spec.Mortal` 的活随从也吃伤害/治疗，不确认、不停帧、不计入胜负。
- 受伤打 3 帧 hit-stop（物理 / 时间 / 感知都停）。打活随从不停帧。致死不换位；胜负等到 hit-stop 结束再判。持有 `NoFrameFreeze` 的单位（及其随从）造成的伤害不停帧。
- `Stun` 期间不发 Sense；碰撞、撞墙、伤害报价仍到。
- 子弹与主人不做 CCD。分身会和本体相撞。只有弹体穿主人，分身不穿。
- 弹体撞上战斗机后 `solid=false`，等角色自己 `Despawn`。`PassWalls` / `BreakWalls` 的弹保持 solid。
- 战斗机死亡时，所有 `owner == 该 id` 的单位一起消失。
- 墙是胶囊段，人人弹开（含自己、分身、子弹）；对敌方战斗机和活随从按 0.1s CD 结算伤害。`BreakWalls` 的弹击中砌墙则该截提前消失并穿过，场边仍撞。
