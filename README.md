...
| `原型机_整合` | 148 | 9999 | 不转向，撞墙弹开。球心青蓝、外圈橙，近身圈半径 185 画橙色细环，不画视野圈。有索敌目标才射，每 0.5s 一发（第一帧就绪），方向在指向目标的 ±60° 内均匀随机。原型机_整合弹 r=6、速 185、伤 2，撞墙第 2 次当帧消失，打中或碰到不可致伤的单位也消失。开火不改自己的速度。敌方战斗机或活随从从圈外跨入时，自身速度改成远离该敌人，并在当下速率上 +70（静止时先按 148）；这种加速叠在巡航之上的部分最多 300，已经到顶就只改方向。快过巡航时球身带拖尾，回到巡航拖尾消失。近身圈平时虚线，圈内有敌方战斗机或活随从才实线。开局已在圈内的可瞄准立即上追踪；之后只有从圈外跨入才上。同一时间一个，5 秒；离开圈不提前解，人没了立刻解，到点不续。已有追踪时不换锁。追踪期间该敌人瞄准优先度改为 1、身上画红圈十字，解开后优先度改回；自己的弹每帧朝被追踪者最多转 1.8°、速率不变，并带拖尾 |
