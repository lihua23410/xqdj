...
unit.Spec{
    ...
    StartHP:   0,                // 0 = 开局满血 MaxHP。小骑士用 75
...
- `Collision` 只在相撞时来。`NX,NY` 是指向对方的法线。`Other` 是对方快照。近战打人通常判断 `Other.Role == RoleFighter`。
- `WallHit` 撞六边形边界或场上墙时来（`PassWalls` 的单位不会撞墙）。
...
`internal/web/style.css` 只保留所有球共用的底：圆、通用 `.glow` / `.trail`、受伤数字、墙。新球的颜色、切片、力场、斩击条都不要写进去。
