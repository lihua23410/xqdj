...
| `原型机_整合` | 148 | 9999 | 不转向，撞墙弹开。球心青蓝、外圈橙。近身圈半径 185 画橙色细环，反向加速圈半径 100 画与球心同色的青色细环；两圈平时虚线，各自圈内有敌方战斗机或活随从才实线。不画视野圈。有索敌目标才射，每 0.5s 一发（第一帧就绪），方向在指向目标的 ±60° 内均匀随机。原型机_整合弹 r=6、速 185、伤 2，撞墙第 2 次当帧消失，打中或碰到不可致伤的单位也消失。开火不改自己的速度。敌方战斗机或活随从从反向加速圈外跨入时，自身速度改成远离该敌人，方向在这条中线的 ±60° 内均匀随机（与开火同一套，每次跨入摇一次），并在当下速率上 +70（静止时先按 148）；这种加速叠在巡航之上的部分最多 300，已经到顶就只改方向。开局已在圈里的不触发，之后直接出现在圈里的也不触发，还在圈里不重复。快过巡航时球身带拖尾，回到巡航拖尾消失。开局已在近身圈内的可瞄准立即上追踪；之后只有从近身圈外跨入才上。从近身圈外一步跨进反向加速圈，同一拍既上追踪也远离。停在两圈之间只上追踪。同一时间一个，5 秒；离开近身圈不提前解，人没了立刻解，到点不续。已有追踪时不换锁，再有人跨进反向加速圈只远离。追踪期间该敌人瞄准优先度改为 1、身上画红圈十字，解开后优先度改回；自己的弹每帧朝被追踪者最多转 1.8°、速率不变，并带拖尾 |
...
| `吹笛人` | 165 | 9999 | 本体不出伤。每 2.5s 吹笛：站定 0.4s、从球边放出一只老鼠，场上最多 6 只。老鼠 r=9、8 血、速 260、视野 9999，平时自己挑最近的敌方「东西」冲上去叼走——敌方战斗机与活随从之外的一切都算（弹、分身、红半/蓝半、面具、鱼塘、月亮、残影、刑具），目标当场消失；叼着东西时降到 180 往回跑，回本体 40 内算入洞：赃物 +1、本体回 3 血、那件东西永久消失。叼东西的老鼠被打死，东西在它倒下的地方还给原主人。指挥（CD 8s）：挥棒站定 0.4s 并吃掉赃物，全场老鼠掉头扑敌各咬一口（基础 3 + 赃物，最多 +3）、持续 2.5s，一次指挥每只只咬一口，咬完立刻退开、不再贴着敌人。老鼠像钉与锤的人偶一样常驻 `Pass`：看得见、打得死，但谁都能穿过它；咬人隔着身位咬、咬完沿径向退开，靠各自的接近角和互相排斥保持队形，不堆成一团。墙偷不走 |
...
非可选单位（不进选人列表，`Fighter: false`）：`子弹`、`分身`、`分身弧`、`缪`、`缪弹`、`缪弧`、`人偶使偶`、`人偶使弹`、`人偶使激光`、`人偶使炸弹`、`原型机_近战弧`、`囚徒囚笼`、`囚徒电椅`、`囚徒绞刑架`、`囚徒钩爪`、`地慧星弧`、`地慧星弹`、`地慧星斩击`、`地慧星残影`、`小骑士弧`、`弱化碎片`、`收割者收回镰刀`、`收割者镰刀`、`教父暗杀者`、`教父狙击弹`、`教父狙击者`、`教父药物`、`教父贩卖者`、`红半`、`蓝半`、`无下限弧`、`无下限术士弧`、`狼人弧`、`狼人撕咬弧`、`狼人月亮`、`盾`、`盾碎片`、`紫弹`、`面灵气弹`、`面灵气面具1`、`面灵气面具2`、`面灵气面具3`、`钓鱼佬中鱼`、`钓鱼佬场鱼塘`、`钓鱼佬大鱼`、`钓鱼佬鱼`、`钓鱼佬鱼塘`、`钉与锤人偶`、`钉与锤弧`、`钉与锤钉`、`原型机_整合弹`、`吹笛人老鼠`。
...
5. **`window.lookFX` 的短名是全局的**，不按包隔离。不要和别的球撞。现在已占用：`assassin`、`axe`、`bond`、`cage`、`chair`、`chroma`、`crescent`、`dealer`、`drug`、`engine`、`fisher`、`fisher-fish`、`fisher-flood`、`fisher-pond`、`gallows`、`glitch`、`glitch-still`、`godfather`、`hammer`、`hammer-doll`、`mask1`、`mask2`、`mask3`、`minion`、`miu`、`miu-arc`、`miu-shot`、`moon`、`nail`、`ningyushi`、`ningyushi-beam`、`ningyushi-bomb`、`ningyushi-doll`、`ningyushi-shot`、`piper`、`prisoner`、`pull`、`push`、`radar`、`rat`、`reaper`、`sickle`、`slash`、`sniper`、`wolf`。一次性特效的名字按包隔离，皮肤短名不会。
