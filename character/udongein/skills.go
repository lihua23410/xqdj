...
func (a *优昙华院) castBreak(ctx unit.Context, s unit.Sense, enemy unit.Snapshot) bool {
	...
	for i := range s.Nearby {
		...
		hit = true
		ctx.Out <- unit.Damage{From: ctx.ID, To: o.ID, Amount: scaled(ctx.ID, breakDamage)}
...
func (a *优昙华院) castLaser(ctx unit.Context, s unit.Sense, enemy unit.Snapshot, act Action) bool {
	...
	ctx.Out <- unit.FX{
		...
		X: a.lockX, Y: a.lockY, VX: ux, VY: uy, Slot: s.Self.Slot,
	}
	return true
}
...
func (a *优昙华院) tickLaser(ctx unit.Context, s unit.Sense) {
	...
	for i := range s.Nearby {
		...
		ctx.Out <- unit.Damage{From: ctx.ID, To: o.ID, Amount: scaled(ctx.ID, laserDamage)}
	}
}
...
func (a *优昙华院) castDose(ctx unit.Context, s unit.Sense) bool {
	...
	if a.doses >= doseMax {
		ctx.Out <- unit.Damage{From: ctx.ID, To: ctx.ID, Amount: doseSuicide}
	}
	return true
}
